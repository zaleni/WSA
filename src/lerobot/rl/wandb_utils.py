...
def cfg_to_group(cfg: TrainPipelineConfig, return_list: bool = False) -> list[str] | str:
    ...
    def short(s: str, max_len: int = 16) -> str:
        """Return a shortened string: keep first max_len chars and replace the rest with '...'."""
        s = re.sub(r"\s+", "_", s)
        if len(s) <= max_len:
            return s
        return s[:max_len] + "_etc"
    ...
    if cfg.dataset is not None:
        # Shorten repo_id if too long to avoid W&B tag length errors
        repo = cfg.dataset.repo_id
        lst.append(f"dataset:{short(repo)}")
...
class WandBLogger:
    ...
    def __init__(self, cfg: TrainPipelineConfig):
        ...
        if wandb.run.get_url():
            ...
            url = Path(wandb.run.dir).parent if hasattr(wandb.run, 'dir') and wandb.run.dir else cfg.output_dir
