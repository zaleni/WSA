...
def make_att_2d_masks(pad_masks, att_masks):  # see openpi `make_att_2d_masks` (exact copy)
    ...
    return (att_2d_masks & pad_2d_masks).to(dtype=torch.bool)
...
class PaliGemmaWithExpertModel(
    ...
    def __init__(
        ...
        vlm_config_hf._vocab_size = 257152  # 257216 257152
        ...
        vlm_config_hf.text_config.vocab_size = 257152  #   # 257216 257152
...
class PI05Policy(PreTrainedPolicy):
    ...
    def __str__(self) -> str:
        ...
        num_vlm = sum(p.numel() for p in self.model.paligemma_with_expert.paligemma.parameters())
        num_act_expert = sum(p.numel() for p in self.model.paligemma_with_expert.gemma_expert.parameters())
        ...
        lines.append(f"  - PaliGemma params    : {num_vlm} ({format_big_number(num_vlm)})")
        lines.append(f"  - Gemma params        : {num_act_expert} ({format_big_number(num_act_expert)})")
    ...
    def _preprocess_images(self, batch: dict[str, Tensor]) -> tuple[list[Tensor], list[Tensor]]:
        ...
        for img_idx in range(3):
            ...
            mask = batch[f"{OBS_IMAGES}.image{img_idx}_mask"].bool()
    ...
    def forward(self, batch: dict[str, Tensor], reduction: str = "mean") -> tuple[Tensor, dict]:
        ...
        loss_dict = {
            "loss_per_dim": l for l in losses.mean(dim=[0, 1]).detach().cpu().numpy().tolist()
        }
