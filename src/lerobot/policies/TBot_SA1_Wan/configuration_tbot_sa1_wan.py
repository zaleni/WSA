...
from lerobot.optim.schedulers import TBotSA1WanNativeSchedulerConfig
...
@DatasetConfig.register_subclass("tbot_sa1_wan")
@dataclass
class TBotSA1WanDatasetConfig(DatasetConfig):
    _canonical_type: ClassVar[str] = TBOT_SA1_WAN

    repo_id: str = "tbot_sa1_wan_local"
...
@PreTrainedConfig.register_subclass("tbot_sa1_wan")
@dataclass
class TBotSA1WanConfig(PreTrainedConfig):
    _canonical_type: ClassVar[str] = TBOT_SA1_WAN

    variant: str = "tbot_sa1_wan"
    ...
    action_dit_pretrained_path: str = "checkpoints/tbot_sa1_wan/ActionDiT_linear_interp_Wan22_alphascale_1024hdim.pt"
    future_3d_pretrained_path: str | None = (
        "checkpoints/tbot_sa1_wan/Future3DExpert_linear_interp_Wan22_alphascale_768hdim.pt"
    )
    ...
    def __post_init__(self) -> None:
        ...
        if self.variant not in {"tbot_sa1_wan"}:
            raise ValueError(
                f"Unsupported TBot_SA1_Wan variant '{self.variant}'. "
                "Expected one of: tbot_sa1_wan."
            )
    ...
    def get_scheduler_preset(self):
        return TBotSA1WanNativeSchedulerConfig(
