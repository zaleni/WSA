...
from .configuration_tbot_sa1_wan import TBotSA1WanConfig
from .core.data.lerobot.utils.normalizer import (
    SingleFieldLinearNormalizer,
    load_dataset_stats_from_json,
)
from .core.models.wan22.tbot_sa1_wan import TBotSA1Wan
from .stats_adapter import ensure_tbot_sa1_wan_stats_format


class TBotSA1WanPolicy(PreTrainedPolicy):
    config_class = TBotSA1WanConfig
    name = TBOT_SA1_WAN
    ...
    def __init__(self, config: TBotSA1WanConfig, *, _defer_config_action_stats: bool = False):
        ...
        if config.native_checkpoint_path:
            logging.info("Loading TBot_SA1_Wan native checkpoint from %s", config.native_checkpoint_path)
    ...
    def _variant_to_class(variant: str):
        mapping = {
            "tbot_sa1_wan": TBotSA1Wan,
        }
        return mapping[variant]

    def _build_model(self, config: TBotSA1WanConfig):
    ...
    def _load_as_safetensor(
        ...
        if unexpected_shape_keys:
            ...
            raise RuntimeError(
                "Unexpected shape mismatch while loading TBot_SA1_Wan pretrained weights: "
                f"{preview}{suffix}"
            )
        ...
        if skipped_shape_keys:
            ...
            suffix = f", ... (+{remaining} more)" if remaining > 0 else ""
            logging.info(
                "Skipping TBot_SA1_Wan shape-mismatched pretrained key(s): %s%s",
                preview,
                suffix,
            )
    ...
    def _save_pretrained(self, save_directory: Path) -> None:
        ...
        if self._action_stats_payload is not None:
            stats_payload = {TBOT_SA1_WAN: self._action_stats_payload, "tbot_sa1_wan": self._action_stats_payload}
    ...
    def from_pretrained(
        ...
        *,
        config: TBotSA1WanConfig | None = None,
        ...
        **kwargs,
        ...
        if config is None:
            config = TBotSA1WanConfig.from_pretrained(
    ...
    @staticmethod
    def _extract_tbot_sa1_wan_stats(stats_payload: dict[str, Any]) -> dict[str, Any]:
        stats_payload = ensure_tbot_sa1_wan_stats_format(
            stats_payload,
            shape_meta=None,
            require_state=False,
        )
        if "action" not in stats_payload:
            raise ValueError("TBot_SA1_Wan stats payload must contain an `action` section.")
    ...
    def set_action_postprocess_from_stats(self, stats_payload: dict[str, Any]) -> None:
        stats_payload = self._extract_tbot_sa1_wan_stats(stats_payload)
        action_stats = stats_payload["action"]
        if not isinstance(action_stats, dict) or len(action_stats) == 0:
            raise ValueError("TBot_SA1_Wan action stats must be a non-empty dict.")
        ...
        for key, key_stats in action_stats.items():
            ...
            if len(selected_stats) == 0:
                raise ValueError(
                    f"Missing {prefix}* stats for TBot_SA1_Wan action key '{key}'. Available keys: {list(key_stats.keys())}"
                )
        ...
        if total_dim > int(self.config.action_dim):
            raise ValueError(
                f"TBot_SA1_Wan action stats dim mismatch: stats sum to {total_dim}, "
                f"which is larger than config.action_dim={self.config.action_dim}"
            )
        if total_dim < int(self.config.action_dim):
            logging.info(
                "TBot_SA1_Wan action stats cover %d/%d dims; trailing policy dims are treated as padded output.",
                total_dim,
    ...
    def _load_action_postprocess_from_stats_path(self, stats_path: str | Path) -> None:
        ...
        if not stats_path.is_file():
            raise FileNotFoundError(f"TBot_SA1_Wan action stats file does not exist: {stats_path}")
        ...
        logging.info("Loaded TBot_SA1_Wan action postprocess stats from %s", stats_path)
    ...
    def _maybe_load_action_postprocess_from_config(
        ...
        *,
        ...
        if errors:
            message = f"TBot_SA1_Wan config action_stats_path is not usable. Last error: {errors[-1]}"
        else:
            message = (
                "TBot_SA1_Wan config action_stats_path was not found. Tried: "
    ...
    def _maybe_load_action_postprocess_from_pretrained(
        ...
        *,
        ...
        if stats_path is None:
            logging.info(
                "TBot_SA1_Wan pretrained source %s has no %s; action outputs will stay normalized unless "
        ...
        try:
            ...
            if (
                ...
                and "TBot_SA1_Wan stats payload must contain an `action` section" not in message
                and "TBot_SA1_Wan action stats dim mismatch" not in message
            ):
                raise
            logging.warning(
                "Skipping automatic TBot_SA1_Wan action stats load from %s: %s. "
    ...
    def _prepare_training_sample(self, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if prompts is None:
            raise ValueError(
                "TBot_SA1_Wan training requires either `context/context_mask` in the batch or `prompt` with "
    ...
    def _resolve_input_image(batch: dict[str, Any]) -> Tensor:
        if "input_image" in batch:
            ...
            raise ValueError("TBot_SA1_Wan inference expects `input_image` or `video` in the batch.")
        if image.ndim == 3:
    ...
    def _denormalize_action(self, action: Tensor) -> Tensor:
        ...
        if action.ndim == 2:
            ...
            raise ValueError(f"Expected TBot_SA1_Wan action to be [B,T,D] or [T,D], got {tuple(action.shape)}")
        ...
        for spec in self._action_denorm_specs:
            ...
            if cur_action.shape[-1] != dim:
                raise ValueError(
                    f"TBot_SA1_Wan action dim mismatch while denormalizing: expected slice dim {dim}, got {cur_action.shape[-1]}"
                )
        ...
        if start > action_f32.shape[-1]:
            raise ValueError(
                f"TBot_SA1_Wan denormalizer needs {start} dims but action has {action_f32.shape[-1]} dims."
            )
