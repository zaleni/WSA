...
class Qwen3VLWithExpertModel(
    ...
    """Qwen3-VL backbone with expert heads for TBotSA1."""
...
class TBotSA1Model(nn.Module):
    ...
    def _apply_lora_to_selected_experts(self):
        ...
        logging.info(
            "Applied TBotSA1 LoRA to experts: %s",
            ", ".join(
    ...
    def __init__(self, config: TBotSA1Config):
        super().__init__()
        self.config = config
    ...
    def gradient_checkpointing_enable(self):
        ...
        logging.info("Enabled gradient checkpointing for TBotSA1 model")
    ...
    def gradient_checkpointing_disable(self):
        ...
        logging.info("Disabled gradient checkpointing for TBotSA1 model")
    ...
    def apply_causal_attention(
        ...
        The default TBotSA1 path never calls this method. In causal mode,
...
class TBotSA1Policy(PreTrainedPolicy):
    """TBotSA1 policy with Qwen3-VL perception, expert action heads, and optional 3D alignment."""

    config_class = TBotSA1Config
    name = TBOT_SA1
    ...
    def __init__(
        ...
        self.model = TBotSA1Model(config)
    ...
    def merge_lora_weights_(self):
        ...
        logging.info("Merged and unloaded %d TBotSA1 LoRA linear modules.", num_merged)
    ...
    def _get_loaded_pretrained_source_config(self) -> TBotSA1Config | None:
        source_config = getattr(self, "_loaded_pretrained_source_config", None)
        return source_config if isinstance(source_config, TBotSA1Config) else None
    ...
    def _validate_lora_loading_compatibility(self) -> bool:
        ...
        if source_config.lora_enabled and not self.config.lora_enabled:
            raise ValueError(
                "The source checkpoint was saved with LoRA adapters enabled, but the current TBotSA1 config "
        ...
        if source_config.lora_enabled and self.config.lora_enabled:
            ...
            if mismatches:
                raise ValueError(
                    "The source LoRA checkpoint is incompatible with the current TBotSA1 LoRA config: "
                    + "; ".join(mismatches)
                )
            ...
            if training_only_mismatches:
                logging.warning(
                    "Loading TBotSA1 LoRA checkpoint with changed training-only settings: %s",
    ...
    def get_state_dict_to_save(self) -> dict[str, Tensor]:
        ...
        logging.info(
            "Saving TBotSA1 checkpoint without frozen external modules: "
