...
from .configuration_tbot_sa1 import TBotSA1Config
from .modeling_tbot_sa1 import TBotSA1Model, TBotSA1Policy, make_att_2d_masks


class TBotSA1RTCModel(TBotSA1Model):
    """TBotSA1 model variant with runtime-only RTC support for inference."""
    ...
    def sample_actions(
        ...
        if self.uses_causal_attention():
            ...
            if rtc_enabled:
                raise ValueError("TBotSA1 RTC inference is not supported with attention_mask_mode='causal'.")
...
class TBotSA1RTCPolicy(TBotSA1Policy):
    """TBotSA1 policy variant that enables RTC only for explicit runtime use."""
    ...
    def __init__(self, config: TBotSA1Config):
        ...
        self.model = TBotSA1RTCModel(config)
