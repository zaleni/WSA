...
from .wsa_large import WSALarge

logger = get_logger(__name__)


class WSALargeJoint(WSALarge):
    """WSA_Large joint variant where the 3D scene also reads future 2D tokens."""
    ...
    def from_wan22_pretrained(cls, **kwargs):
        ...
        if not isinstance(video_dit_config, dict):
            raise ValueError(
                "`video_dit_config` must be provided as dict for WSALargeJoint."
            )
        if bool(video_dit_config.get("action_conditioned", False)):
            raise ValueError(
                "WSALargeJoint requires `video_dit_config['action_conditioned']=false`."
            )
    ...
    def infer_joint(
        ...
        if test_action_with_infer_action:
            logger.warning(
                "`WSALargeJoint.infer_joint` ignores `test_action_with_infer_action=True` "
