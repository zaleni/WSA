...
class MetricsTracker:
    ...
    def step(self) -> None:
        ...
        """
        self.steps += 1
        # _batch_size is already the effective batch size (per-process batch * num_processes)
        # when initialized with effective_batch_size in distributed training
    ...
    def __str__(self) -> str:
        display_list = [
            ...
            f"sample:{format_big_number(self.samples)}",
            # number of episodes seen during training
            f"episode:{format_big_number(self.episodes)}",
            # number of time all unique samples are seen
            f"epoch:{self.epochs:.2f}",
