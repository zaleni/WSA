...
POLICY_INIT_PATH="${POLICY_INIT_PATH:-${PRETRAINED_PATH:-zaleni/TBot-SA1-Base}}"
...
if [[ "${TBOT_SA1_ATTENTION_MASK_MODE}" == "causal" ]]; then
  echo "TBOT_SA1_ATTENTION_MASK_MODE=causal detected; using causal."
  TBOT_SA1_ATTENTION_MASK_MODE="causal"
fi
...
if [[ "${TBOT_SA1_ATTENTION_MASK_MODE}" == "causal" && "${ENABLE_3D_QUERIES}" != "true" ]]; then
  echo "TBOT_SA1_ATTENTION_MASK_MODE=causal requires ENABLE_3D_QUERIES=true."
  exit 1
fi
...
JOB_NAME="${JOB_NAME:-${POLICY}-robotwin-${ACTION_TYPE}-chunk${CHUNK_SIZE}-${INIT_TAG}-${TBOT_SA1_ATTENTION_MASK_MODE}-gen${LAMBDA_GEN}-3d${LAMBDA_3D}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
