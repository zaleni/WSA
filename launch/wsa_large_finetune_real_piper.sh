...
DATASET_DIR="${DATASET_DIR:-/path/to/Rank_Block_RGB_lerobot_v30}"
...
RAW_ACTION_DIM="${RAW_ACTION_DIM:-7}"
RAW_PROPRIO_DIM="${RAW_PROPRIO_DIM:-7}"
NUM_INFERENCE_STEPS="${NUM_INFERENCE_STEPS:-10}"
NUM_FRAMES="${NUM_FRAMES:-49}"
ACTION_HORIZON="$((NUM_FRAMES - 1))"
N_ACTION_STEPS="$((NUM_FRAMES - 1))"
...
FUTURE_3D_VIEW_ATTENTION_LAYOUT="${FUTURE_3D_VIEW_ATTENTION_LAYOUT:-horizontal}"
...
IMAGE_KEYS="${IMAGE_KEYS:-[\"head\",\"left\"]}"
IMAGE_RAW_SHAPES="${IMAGE_RAW_SHAPES:-[[3,480,640],[3,480,640]]}"
IMAGE_SHAPES="${IMAGE_SHAPES:-[[3,240,320],[3,240,320]]}"
...
PROCESSOR_DELTA_ACTION_DIM_MASK="${PROCESSOR_DELTA_ACTION_DIM_MASK:-{\"default\":[true,true,true,true,true,true,false]}}"
...
if [[ "${robot_type}" != "real_piper" ]]; then
  echo "Expected robot_type=real_piper, got ${robot_type}"
...
if [[ "${VALIDATE_DATASETS}" == "true" ]]; then
  echo "Validating Real_Piper dataset mappings..."
...
if missing:
    raise SystemExit(f"Missing required WSA_Large Real_Piper features: {missing}")
...
JOB_NAME="${JOB_NAME:-${WSA_LARGE_VARIANT}-real_piper-${DATASET_NAME}-${ACTION_TYPE}-${BOOTSTRAP_TAG}-finetune-$(date +'%Y_%m_%d_%H_%M_%S')}"
...
if [[ "${LOAD_TEXT_ENCODER}" == "true" ]]; then
  ...
  TEXT_EMBED_CACHE_DIR="${TEXT_EMBED_CACHE_DIR:-${PROJ_ROOT}/outputs/WSA_Large/text_embeds/real_piper/${DATASET_NAME}}"
fi
...
ARGS=(
    ...
    --dataset.type=${POLICY}
    --dataset.repo_id="real_piper_from_file"
    ...
    --wandb.enable=true
    --wandb.project=WSA_Large_RealPiper
