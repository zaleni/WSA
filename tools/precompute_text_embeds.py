...
from lerobot.policies.WSA_Large.core.models.wan22.helpers.loader import (
    _load_registered_model,
    _resolve_configs,
)
from lerobot.policies.WSA_Large.core.models.wan22.wan_video_text_encoder import HuggingfaceTokenizer
from lerobot.policies.WSA_Large.text_cache import (
    build_wsa_large_prompt,
...
def _read_task_strings_from_parquet(dataset_dir: Path) -> list[str]:
    ...
    task_values = tasks["task"].tolist() if "task" in tasks.columns else tasks.index.tolist()
    return [str(task).strip() for task in task_values if str(task).strip()]
...
def _read_unique_prompts(dataset_dirs: list[str]) -> list[str]:
    ...
    for ds_dir in dataset_dirs:
        for task in _read_task_strings(Path(ds_dir)):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Precompute WSA_Large offline text embedding cache.")
    parser.add_argument(
        "--dataset-dir",
        action="append",
        default=[],
    ...
    parser.add_argument("--repo-id-file", default=None, help="Optional file with one dataset directory per line.")
    parser.add_argument(
        "--text-embedding-cache-dir",
        required=True,
        help="Output cache directory used later by WSA_Large training.",
    )
    ...
    if verify_cache_only:
        ...
        if (not is_distributed) and torch.cuda.is_available() and torch.cuda.device_count() > 1:
            logging.info(
                ...
                "tools/precompute_text_embeds.py ...",
    ...
    if has_override_instruction:
        prompts = [build_wsa_large_prompt(str(args.override_instruction).strip())]
