...
WSA_LARGE_VIDEO_HEIGHT="${WSA_LARGE_VIDEO_HEIGHT:-384}"
WSA_LARGE_VIDEO_WIDTH="${WSA_LARGE_VIDEO_WIDTH:-320}"
WSA_LARGE_STANDARDIZE_VIDEO_SIZE_BY_CAMERAS="${WSA_LARGE_STANDARDIZE_VIDEO_SIZE_BY_CAMERAS:-true}"
WSA_LARGE_CONCAT_MULTI_CAMERA="${WSA_LARGE_CONCAT_MULTI_CAMERA:-robotwin}"
...
if [[ -z "${CHECKPOINT_DIR}" ]]; then
  echo "Please set CHECKPOINT_DIR to a WSA_Large checkpoint step dir or pretrained_model dir."
  exit 1
fi

case "${RTC_ENABLED,,}" in
  true|1|yes|y|on)
    echo "WSA_Large Real_Lift2 RTC serving is not implemented yet. Use sync mode with RTC_ENABLED=false."
    exit 1
    ;;
  false|0|no|n|off)
    ;;
